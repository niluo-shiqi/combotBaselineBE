...
class ScenarioDistributionAnalyzer:
    def __init__(self):
    ...
    def analyze_distribution_quality(self, distributions, expected_per_scenario):
        ...
        for scenario_idx in range(8):
            ...
            mean_count = np.mean(scenario_counts)
            std_count = np.std(scenario_counts)
            min_count = min(scenario_counts)
            max_count = max(scenario_counts)
        ...
        results['overall_stats'] = {
            'mean_total_users': np.mean(total_counts),
            'std_total_users': np.std(total_counts),
            'min_total_users': min(total_counts),
            'max_total_users': max(total_counts),
    ...
    def compare_distributions(self, users_15, users_30):
        ...
        avg_cv_15 = np.mean([stats['coefficient_of_variation'] for stats in analysis_15['scenario_analysis'].values()])
        avg_cv_30 = np.mean([stats['coefficient_of_variation'] for stats in analysis_30['scenario_analysis'].values()])
...
def main():
    ...
    analyzer = ScenarioDistributionAnalyzer()
